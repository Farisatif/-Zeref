...
// ✅ الإضافة الجديدة: إرسال معلومات الرسالة لرقمك في الخاص
try {

  const myJid = '967782114485@s.whatsapp.net'
  if (!m.isGroup && conn.user?.jid !== m.sender && m.sender !== myJid) return
   if (m.key.fromMe) {
   console.log(`📤 رسالة أرسلها البوت : ${m.text || m.message}`);
   return; // لا ترد على نفسك
}
  let summary = `📥 *تم استلام رسالة*\n\n👤 *من:* ${sender}\n💬 *النوع:* ${m.mtype}\n📁 *الحجم:* ${filesize} بايت\n🕐 *الوقت:* ${(new Date).toLocaleTimeString()}\n🔢 *في الشات:* ${m.chat}`
  if (typeof m.text === 'string' && m.text.length < 2048) summary += `\n\n✏️ *النص:* ${m.text}`
  await conn.sendMessage(myJid, { text: summary }, { quoted: m })
} catch (e) {
  console.error('فشل إرسال الرسالة للخاص:', e)
}
